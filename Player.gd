...
func update_weapon() -> void:
	if current_weapon == -1:
		for node in inventory.get_children():
			node.queue_free()
	var res : ResourceWeapon = GlobalResourceLoader.item_cache[weapon_resource_ids[current_weapon]]
...
func merge_weapon(target_res_id : String) -> void:
	var target_res : ResourceWeapon = GlobalResourceLoader.item_cache[target_res_id]
