...
func dissolve() -> void:
	...
	dissolve_tween.set_parallel()
	dissolve_tween.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
	dissolve_tween.tween_property(material, "shader_parameter/dissolve_value", 0.0, 0.7)
	dissolve_tween.tween_property(texture_rect.material, "shader_parameter/dissolve_value", 0.0, 0.7)
...
func reset_dissolve() -> void:
	...
	if texture_rect and texture_rect.material:
		...
		texture_rect.material.set_shader_parameter("dissolve_value", 1.0)
