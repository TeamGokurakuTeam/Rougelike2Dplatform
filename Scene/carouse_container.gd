...
func _process(delta: float) -> void:
	...
	for node in position_offset_node.get_children():
		...
		if wraparound_enabled:
			...
			var angle = clamp((control_node.get_index() - selected_index) / max_index_range, -1.0, 1.0) * PI
			...
			var position_x = 0
			if control_node.get_index() > 0:
				position_x = position_offset_node.get_child(control_node.get_index() - 1).position.x + position_offset_node.get_child(control_node.get_index() - 1).size.x + spacing
			control_node.position = Vector2(position_x, -control_node.size.y / 2.0)
	
		control_node.pivot_offset = control_node.size / 2.0
		var target_scale = 1.0 - (scale_strength * abs(control_node.get_index() - selected_index))
		...
		var target_opacity = 1.0 - (opacity_strength * abs(control_node.get_index() - selected_index))
