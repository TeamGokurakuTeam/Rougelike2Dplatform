...
func _ready() -> void:
	...
	outside_tween.parallel().tween_property(out_side, "self_modulate", Color("ffffff2e"), 0.3).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	...
	in_side_tween.parallel().tween_property(out_side, "self_modulate", Color("ffffff2e"), 0.6).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
